            ...
            <div class="col-12">
                <div class="card mb-4">
                    <div class="d-flex justify-content-between">
                        <a href="{{ route('create', $beneficiaire->id) }}"
                            class="btn btn-m btn-link mx-auto mb-0 d-none d-lg-block"><i
                                class="fas fa-plus"></i>&nbsp;&nbsp;اضافة مخالفة</a>
                    </div>
                    <div class="card-header pb-0">
                        <h6>جدول المخالفات</h6>
                    </div>
                                    ...
                                    @foreach ($infraction as $item)
                                        <tr>
                                            ...
                                            <td class="align-middle text-center text-sm">
                                                <p class="text-xs font-weight-bold mb-0">{{ $item->decision }}</p>
                                            </td>
                                            <td class="align-middle text-center">
                                                <div class="ml-auto text-center">
                                                    <form id="deleteForm" onsubmit="return confirmDelete()"
                                                        action="{{ route('infraction.destroy', $item->id) }}"
                                                        method="post">
                                                        @csrf
                                                        @method('DELETE')
                                                        <button
                                                            class="btn btn-link text-center text-danger text-gradient px-0 mb-0"><i
