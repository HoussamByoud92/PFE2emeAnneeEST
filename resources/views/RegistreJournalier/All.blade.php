            ...
            <div class="col-12">
                <div class="card mb-4">
                    <div class="d-flex justify-content-between">
                        <a href="{{ route('creat', $beneficiaire->id) }}"
                            class="btn btn-m btn-link mx-auto mb-0 d-none d-lg-block"><i
                                class="fas fa-plus"></i>&nbsp;&nbsp;اضافة تسجيل يومي</a>
                    </div>
                    <div class="card-header pb-0">
                        <h6>جدول التسجيلات اليومية</h6>
                    </div>
                    <div class="card-body px-0 pt-0 pb-2">
                        <div class="table-responsive p-0">
                            <table class="table align-items-center mb-0">
                                <thead>
                                    <tr>
                                        <th class="text-uppercase text-secondary text-xs font-weight-bolder opacity-7">تاريخ
                                        </th>
                                        <th
                                ...
                                <tbody>
                                    @foreach ($registreJournalier as $item)
                                        <tr>
                                            <td>
                                            ...
                                            <td class="align-middle text-center text-sm">
                                                <p class="text-xs font-weight-bold mb-0">{{ $item->abscence }}</p>
                                            </td>
                                            <td class="align-middle text-center text-sm">
                                                <p class="text-xs font-weight-bold mb-0">{{ $item->periode_abs }}</p>
                                            </td>
                                            <td class="align-middle text-center text-sm">
                                                <p class="text-xs font-weight-bold mb-0">{{ $item->justife }}</p>
                                            </td>
                                            <td class="align-middle text-center text-sm">
                                                <p class="text-xs font-weight-bold mb-0">{{ $item->activite }}</p>
                                            </td>
                                            <td class="align-middle text-center text-sm">
                                                <p class="text-xs font-weight-bold mb-0">{{ $item->type_act }}</p>
                                            </td>
                                            <td class="align-middle text-center text-sm">
                                                <p class="text-xs font-weight-bold mb-0">{{ $item->heure_act }}</p>
                                            </td>
                                            <td class="align-middle text-center text-sm">
                                                <p class="text-xs font-weight-bold mb-0">{{ $item->incident }}</p>
                                            </td>
                                            <td class="align-middle text-center text-sm">
                                                <p class="text-xs font-weight-bold mb-0">{{ $item->type_incid }}</p>
                                            </td>
                                            <td class="align-middle text-center text-sm">
                                                <p class="text-xs font-weight-bold mb-0">
                                                    {{ $item->description_incid }}</p>
                                            </td>
                                            <td class="align-middle">
                                                <div class="ml-auto text-end row">
                                                    <form id="deleteForm" onsubmit="return confirmDelete()" class="col-4"
                                                        action="{{ route('registreJournalier.destroy', $item->id) }}"
                                                        method="post">
                                                        @csrf
                                                        @method('DELETE')
